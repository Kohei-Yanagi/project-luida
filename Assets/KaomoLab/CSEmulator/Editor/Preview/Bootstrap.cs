...
namespace Assets.KaomoLab.CSEmulator.Editor.Preview
{
    ...
    {
        public static event Action<LogSource, LogLevel, ScriptMessageLoggedArgs> OnScriptMessageLogged = delegate { };
        public static event Action<LogSource, LogLevel, ScriptLoggedArgs> OnScriptLogged = delegate { };
        ...
        readonly static Engine.ILoggerFactory itemScriptLoggerFactory;
        readonly static Engine.ILoggerFactory playerScriptLoggerFactory;
        ...
        static Bootstrap()
        {
            ...
            itemScriptLoggerFactory = new DebugLogFactory(
                BuildOnScriptMessageLogged(LogSource.ItemScript),
                BuildOnScriptLogged(LogSource.ItemScript),
                options
            );
            playerScriptLoggerFactory = new DebugLogFactory(
                BuildOnScriptMessageLogged(LogSource.PlayerScript),
                BuildOnScriptLogged(LogSource.ItemScript),
                options
            );
        }

        static Action<LogLevel, ScriptMessageLoggedArgs> BuildOnScriptMessageLogged(LogSource logSource)
        {
            return new Action<LogLevel, ScriptMessageLoggedArgs>((logLevel, args) =>
            {
                OnScriptMessageLogged.Invoke(logSource, logLevel, args);
            });
        }
        static Action<LogLevel, ScriptLoggedArgs> BuildOnScriptLogged(LogSource logSource)
        {
            return new Action<LogLevel, ScriptLoggedArgs>((logLevel, args) =>
            {
                OnScriptLogged.Invoke(logSource, logLevel, args);
            });
        }
        ...
        static void StartCSEmulator()
        {
            ...
            engine = new EngineFacadeFactory(
                optionBridge, oscServer, itemScriptLoggerFactory, playerScriptLoggerFactory
            ).CreateDefault();
            engine.Start();
