...
namespace Assets.KaomoLab.CSEmulator.Editor.Engine
{
        ...
        : EmulateClasses.IPlayerController, EmulateClasses.IPlayerTransformController
    {
        ...
        {
            get => playerOptions.isFirstPersonView;
            set
            {
                if (isCameraPositionSetted) return;
                playerOptions.isFirstPersonView = value;
            }
        }
        ...
        bool isCameraPositionSetted = false;
        bool isPerspactiveLocking = false;
        ...
        void ApplyPersonViewChangeable()
        {
            playerOptions.SetPersonViewChangeable(!(isCameraPositionSetted || isPerspactiveLocking));
        }
        public void SetPerspectiveSwitchingLocked(bool isLocked)
        {
            isPerspactiveLocking = isLocked;
            ApplyPersonViewChangeable();
        }
        ...
        public void SetCameraPosition(Vector3? position)
        {
            if(position == null)
            {
                ...
                ApplyPersonViewChangeable(); //勝手に解除になるので最後で反映させる。
            }
            else
            {
                isCameraPositionSetted = true;
                if (firstPersonViewResetTo == null)
                {
                    //人称が切り替える前に保存する
